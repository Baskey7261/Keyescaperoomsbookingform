(function(root, window, document, factory, undefined) {
    if (typeof define === 'function' && define.amd) {
        ...
        define(function() {
            root.tvCalendar = factory(window, document);
            return root.tvCalendar;
        });
        ...
        // Browser globals.
        window.tvCalendar = factory(window, document);
    }
})(this, window, document, function(window, document) {
    'use strict';

    var CLASS_CALENDAR = "tv-calendar";
    var CLASS_CALENDAR_INFO = "tv-calendar__info";
    var CLASS_CALENDAR_CODE = "tv-calendar__code";
    var CLASS_CALENDAR_CODE_LOCK = "tv-calendar__code_lock";
    var CLASS_CALENDAR_HEADER = "tv-calendar__header";
    var CLASS_CALENDAR_NAV = "tv-calendar__nav";
    var CLASS_CALENDAR_NAV_PREV = "tv-calendar__nav_prev";
    var CLASS_CALENDAR_NAV_NEXT = "tv-calendar__nav_next";
    var CLASS_CALENDAR_RESET = "tv-calendar__reset";
    var CLASS_CALENDAR_SELECT_DATE = "tv-calendar__select-date";
    var CLASS_CALENDAR_SELECT_START = "tv-calendar__select-start";
    var CLASS_CALENDAR_SELECT_END = "tv-calendar__select-end";
    var CLASS_CALENDAR_WEEK_NAMES = "tv-calendar__week-names";
    var CLASS_CALENDAR_WEEKDAY = 'tv-calendar__weekday'
    var CLASS_CALENDAR_MONTH = "tv-calendar__month-label";
    var CLASS_CALENDAR_DAYS = "tv-calendar__days";
    var CLASS_CALENDAR_DAY = "tv-calendar__day";
    var CLASS_CALENDAR_INNER = "calendarDaysInner";
    var CLASS_CALENDAR_DAY_REL_FUTURE = "tv-calendar__day_rel-future";
    var CLASS_CALENDAR_DAY_REL_PAST = "tv-calendar__day_rel-past";
    var CLASS_CALENDAR_DAY_REL_TODAY = "tv-calendar__day_rel-today";
    var CLASS_CALENDAR_DAY_ABS_PAST = "tv-calendar__day_abs-past";
    var CLASS_CALENDAR_DAY_ABS_FUTURE = "tv-calendar__day_abs-future";
    var CLASS_CALENDAR_DAY_ABS_TODAY = "tv-calendar__day_abs-today";
    var CLASS_CALENDAR_DAY_SELECT = "tv-calendar__day_select";
    var CLASS_CALENDAR_DAY_RANGE_SELECT = "tv-calendar__day_range-select";
    var CLASS_CALENDAR_DAY_RANGE_START = "tv-calendar__day_range-start";
    var CLASS_CALENDAR_DAY_RANGE_END = "tv-calendar__day_range-end";
    var CLASS_CALENDAR_DAY_OFF = "tv-calendar__day_off";
    var CLASS_CALENDAR_DAY_LOCK = "tv-calendar__day_lock";
    var CLASS_CALENDAR_DAY_DIFFERENT_MONTH = "tv-calendar__day_different-month";
    var CLASS_CALENDAR_DAY_HIGHTLIGHT = "tv-calendar__day_highlight";


    function showError(type, text) {
        window.console && window.console[type] && window.console[type]('tvCalendar: ' + text);
    }

    function getDummyDay() {
        var dummy_day_el, dummy_dayContainer;

        dummy_dayContainer = document.createElement('span');
        dummy_dayContainer.className = CLASS_CALENDAR_DAY + " " + CLASS_CALENDAR_DAY_DIFFERENT_MONTH;

        dummy_day_el = document.createElement("span");
        dummy_day_el.className = CLASS_CALENDAR_INNER;
        dummy_day_el.textContent = "";

        dummy_dayContainer.appendChild(dummy_day_el);

        return dummy_dayContainer;
    }
    ...
    var tvCalendar = function(container_q, user_options) {
    ...
    tvCalendar.prototype.mount = function() {
        ...
        //var mnth = calendar_month_el.textContent.split(",");
        ...
        for (var d = 1; d <= days_in_month; d++) {
            ...
            if (moment_copy.isSame(moment(), "day")) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_ABS_TODAY;
            } else if (moment_copy.isBefore(moment(), "day")) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_ABS_PAST;
            } else if (moment_copy.isAfter(moment(), "day")) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_ABS_FUTURE;
            }

            //RELATIVE RELATIONS
            if (moment_copy.isSame(moment(this.state.date, this.config.format), "day")) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_REL_TODAY;
            } else if (moment_copy.isBefore(moment(this.state.date, this.config.format), "day")) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_REL_PAST;
            } else if (moment_copy.isAfter(moment(this.state.date, this.config.format), "day")) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_REL_FUTURE;
            }

            //MEANINGFUL MARKERS
            if (this.state.highlight.indexOf(moment_copy.format(this.config.format)) > -1) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_HIGHTLIGHT;
            }
            ...
            if (this.state.blacklist.indexOf(moment_copy.format(this.config.format)) > -1) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_OFF + " " + CLASS_CALENDAR_DAY_LOCK;
            }

            if (this.state.selected.indexOf(moment_copy.format(this.config.format)) > -1) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_SELECT
            }

            //Range select
            if (this.state.date_start && moment_copy.isSame(this.state.date_start, 'day')) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_RANGE_START + " " + CLASS_CALENDAR_DAY_RANGE_SELECT + " " + CLASS_CALENDAR_DAY_SELECT;
            }

            if (this.state.date_start && !this.state.date_end && moment_copy.isBefore(moment(this.state.date_start, this.config.format), "day")) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_LOCK
            }

            if (this.state.date_end && moment_copy.isSame(this.state.date_end, 'day')) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_RANGE_END + " " + CLASS_CALENDAR_DAY_RANGE_SELECT + " " + CLASS_CALENDAR_DAY_SELECT;
            }
            ...
            if ((this.state.date_start && this.state.date_end && moment_copy.isBetween(moment(this.state.date_start, this.config.format), moment(this.state.date_end, this.config.format), 'day'))) {
                dayContainer.className = dayContainer.className + " " + CLASS_CALENDAR_DAY_SELECT + " " + CLASS_CALENDAR_DAY_RANGE_SELECT
            }
            ...
            dayContainer.appendChild(day_el);
            calendar_days_el.appendChild(dayContainer);

            moment_copy.add(1, "d");
        }

        // set back for future use
    ...
    tvCalendar.prototype.dayClick = function(date, day_el) {
    ...
    tvCalendar.prototype.getSelected = function() {
        return this.state.selected;
    }

    tvCalendar.prototype.getStartDate = function() {
        return this.state.date_start;
    }

    tvCalendar.prototype.getEndDate = function() {
        return this.state.date_end;
    }

    tvCalendar.prototype.getRange = function() {
        return {
    ...
    tvCalendar.prototype.getFocusYear = function() {
        return this.moment.format('YYYY');
    }

    tvCalendar.prototype.getFocusMonth = function() {
        return this.moment.format('MM');
    }

    tvCalendar.prototype.getFocusDay = function() {
        return this.moment.format('DD');
    }

    tvCalendar.prototype.getConfig = function() {
        return this.config;
    }

    tvCalendar.prototype.getState = function() {
        this.state.date_calendar = this.moment.format(this.config.format);

        return this.state;
    }

    tvCalendar.prototype.sync = function(obj) {
    ...
    tvCalendar.prototype.nextMonth = function(e) {
    ...
    tvCalendar.prototype.prevMonth = function(e) {
    ...
    tvCalendar.prototype.reset = function() {
    ...
    tvCalendar.prototype.removeLock = function() {
        this.state.lock = false;
        this.elements.calendar_code.classList.remove(CLASS_CALENDAR_CODE_LOCK);
    }

    tvCalendar.prototype.bindEvents = function() {
        var that = this;
    ...
    tvCalendar.prototype.destroy = function() {
...
/**
 * jQuery adapter for tvCalendar
 */
if (window.jQuery && window.tvCalendar) {
    (function($, tvCalendar) {
        'use strict';

        $.fn.tvCalendar = function(options) {
            return new tvCalendar(this[0], options);
        };
    })(window.jQuery, window.tvCalendar);
}